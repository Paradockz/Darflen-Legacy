...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if ($author = get_user_info_from_identifier($identifier)) {
            $total = $database->preparedQuery('SELECT count(id) as result FROM follows WHERE follower = ?', [$author['id']])->fetch(PDO::FETCH_ASSOC)['result'];
            $paginator = 15;
            ...
            $follows = $database->preparedQuery('SELECT follower FROM follows WHERE follower = ? LIMIT ?,?', [$author['id'], $paginator * $paginator_page, $paginator])->fetchAll(PDO::FETCH_ASSOC);
            $errors['code'] = 'success';
