...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if ($post = get_reply_info_from_id($identifier)) {
            $post_data = json_decode($post['data'], true);
            $total = $database->preparedQuery('SELECT count(id) as result FROM subreplies WHERE reply = ?', [$post['id']])->fetch(PDO::FETCH_ASSOC)['result'];
            $paginator = 15;
            ...
            $replies = $database->preparedQuery('SELECT id, author, text, data FROM subreplies WHERE reply = ? ORDER BY JSON_VALUE(data,"$.miscellaneous.creation_time") ASC LIMIT ?,?', [$post['id'], $paginator * $paginator_page, $paginator])->fetchAll(PDO::FETCH_ASSOC);
            $errors['code'] = 'success';
