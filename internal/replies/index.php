        ...
        <div id="internal-contents">
            <div class="internal-section">
                <ul class="internal-micro-stats">
                    <?php
                    internalMicroStat('Replies', $data['subreplies'], 'message-alt.svg');
                    ?>
                </ul>
            </div>
            <div class="internal-section">
                    ...
                    <div class="internal-post">
                        <div class="internal-actions">
                            <ul class="internal-table-list">
                                ...
                                <li>
                                    <button class="lb-button internal-table-button" onclick="disappear2('<?php echo $post['id'] ?>', 'Reply');return false">
                        ...
                        echo build_subreply($post, $user, $yourself, $loved, $loves, false, false);
                        ?>
                    </div>
                <?php
                }
                ...
                <div id="internal-paginator">
                    <div class="internal-paginator-section">
                        <ul id="profile-posts-paginator">
                            ...
                            generate_paginator_3($pg, 2, $user, 'replies');
                            generate_paginator_3($pg, 3, $user, 'replies');
                            generate_paginator_4($pg, $paginator_count - 3, $user, 'replies');
                            generate_paginator_4($pg, $paginator_count - 2, $user, 'replies');
                            generate_paginator_4($pg, $paginator_count - 1, $user, 'replies');
                            if ($pg + 1 < $paginator_count) : $p = $pg + 1 ?>
                                <li class="profile-posts-paginator-page internal-paginator">
                                    <a href="<?php echo ROOT_LINK ?>/internal/replies/<?php echo '?page=' . $p ?>">></a>
                                </li>
                            <?php endif ?>
                        </ul>
                    </div>
                    <div class="internal-paginator-section">
                        <p class="internal-paginator-info">Showing reports <?php echo $paginator_page * $paginator + 1 ?> to <?php echo ($paginator_page * $paginator + $paginator) > $data['subreplies'] ? $data['subreplies'] : ($paginator_page * $paginator + $paginator) ?> in <?php echo $data['subreplies'] ?></p>
                    </div>
                </div>
            </div>
        </div>
