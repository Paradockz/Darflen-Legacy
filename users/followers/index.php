...
$total_followers = $database->preparedQuery('SELECT count(id) AS followers FROM follows WHERE following = ?', [$id])->fetch(PDO::FETCH_ASSOC)['followers'];
$paginator = 36;
...
$followers = $database->preparedQuery('SELECT follower FROM follows WHERE following = ? LIMIT ?,?', [$id, $paginator * $paginator_page, $paginator])->fetchAll(PDO::FETCH_ASSOC);
?>

<div id="content">
    <div id="profile-followers">
        <div class="profile-followers-section">
            <h1><?php echo json_decode($user['data'], true)['username'] ?>'s Followers</h1>
            <ul id="stats-accounts-grid">
                <?php
                foreach ($followers as $follower) {
                ?>
                    <li class="stats-account"><?php userCard($follower['follower']) ?></li>
                <?php
                }
                ?>
            </ul>
            ...
            ?>
                <p id="empty-stats"><?php echo json_decode($user['data'], true)['username'] ?> doesn't have anyone following them yet! Come back later to see if they've got some followers.</p>
            <?php
            }
            ?>
        </div>
            ...
            <ul id="profile-posts-paginator">
                ...
                $pg = $_GET['page'] ?? 1; if ($pg > 1): $p = $pg - 1 ?>
                     <li class="profile-posts-paginator-page">
                        <a href="<?php echo ROOT_LINK ?>/users/<?php  echo $user['identifier'] ?>/followers<?php echo '?page='.$p ?>"><</a>
                    </li>
                <?php endif;
